...
function MidiGrid() {
    const { selectedNote, setSelectedNote, cols, setCols } =
    ...
    const updateNotePosition = (id: string, row: number, col: number) => {
        setMidi((prevItems) => {
    ...
    const handleEdgeDrag = useCallback(
        (col: number) => {
            if (col >= cols - 1) {
                setCols((prevCols) => prevCols + 10); // Add more columns
            }
        },
        [cols]
    );
    ...
    const [, drop] = useDrop(
        () => ({
            ...
            drop(item: MidiNoteType, monitor) {
                ...
                const newCol = item.col + colChange;
                handleEdgeDrag(newCol);

                updateNotePosition(item.id, rowChange, colChange);
                return undefined;
            },
        }),
        [updateNotePosition]
    ...
    const gridItems = midiNotes.flatMap((note, rowIndex) =>
        Array.from({ length: cols }).map((_, colIndex) => (
            <div
                ...
                style={{ height: "18.09px", width: "28.92px", zIndex: 1 }}
                onMouseDown={(e) => handleOnClick(e, note, rowIndex, colIndex)}
                onMouseMove={(e) =>
                    handleOnMouseMove(e, note, rowIndex, colIndex)
                }
